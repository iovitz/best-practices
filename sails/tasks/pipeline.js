...
module.exports.cssFilesToInject = cssFilesToInject.map((cssPath) => {
  ...
  if (cssPath[0] === '!') {
    return require('node:path').join(`!${tmpPath}`, cssPath.substr(1))
  }
  return require('node:path').join(tmpPath, cssPath)
})
module.exports.jsFilesToInject = jsFilesToInject.map((jsPath) => {
  // If we're ignoring the file, make sure the ! is at the beginning of the path
  if (jsPath[0] === '!') {
    return require('node:path').join(`!${tmpPath}`, jsPath.substr(1))
  }
  return require('node:path').join(tmpPath, jsPath)
})
module.exports.templateFilesToInject = templateFilesToInject.map((tplPath) => {
  // If we're ignoring the file, make sure the ! is at the beginning of the path
  if (tplPath[0] === '!') {
    return require('node:path').join('!assets/', tplPath.substr(1))
  }
  return require('node:path').join('assets/', tplPath)
})
